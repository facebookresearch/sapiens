...
START_INDEX=0
END_INDEX=6200 ## for safety
NUM_NODES=4

# START_INDEX=0
# END_INDEX=40 ## for safety
# NUM_NODES=2

##-------------------------------------------------------------------------
RANGE_PER_NODE=$(( (END_INDEX - START_INDEX) / NUM_NODES ))

OUTPUT_DIR="/home/rawalk/Desktop/sapiens/pose/Outputs/pose_process/slurm_convert"
OUTPUT_DIR="$(echo "${OUTPUT_DIR}/$(date +"%m-%d-%Y_%H:%M:%S")")"
CONDA_ENV='/uca/conda-envs/dgxenv-2023-09-25-7853/bin/activate'
TIME='7-00:00:00'
JOB_NAME="cca_convert_pose"
...
./convert_valid_slurm_helper.sh $START_SAMPLE_ID $END_SAMPLE_ID
EOL
