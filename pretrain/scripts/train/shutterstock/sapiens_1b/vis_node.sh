...
RUN_FILE='./tools/vis_dist_train.sh'
PORT=$(( ((RANDOM<<15)|RANDOM) % 63001 + 2000 ))

# ###----------------------------------------------------
DATASET='shutterstock'
MODEL="train_vis_mae_sapiens_1b-p16_8xb512-coslr-1600e_${DATASET}"
...
if [ "$mode" = "debug" ]; then
    ...
    CUDA_VISIBLE_DEVICES=${DEVICES} python tools/vis_train.py ${CONFIG_FILE} --work-dir ${OUTPUT_DIR} ${CMD_RESUME} --no-validate --cfg-options ${OPTIONS}
