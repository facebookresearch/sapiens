...
instagram_dataset = dict(
        type='Instagram',
        data_root='data/instagram',
        airstore_id='codec_avatars_ig_image_people_90days_source20240222_v0',
        split='train',
        pipeline=train_pipeline)
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        datasets=[shutterstock_dataset, instagram_dataset]
    ))
...
strategy = dict(
    type='FSDPStrategy',
    # state_dict_cfg='full', ## store the full state dict in rank 0. this is timing out
    state_dict_cfg='local', ## save all ranks respectively
    model_wrapper=dict(
