...
class GeneralInstagram(AirstoreBaseDataset):
    METAINFO = {'classes': 'person'}

    def __init__(self,
                 data_root: str = '',
                 airstore_id: str = '',
                 concepts_def_path: str = '',
                 split: str = '',
        ...
        self.concepts_def_path = concepts_def_path
        df = pd.read_csv(concepts_def_path)
        concepts_id_to_name = (
            df[["Concept Name", "Concept ID"]]
            .set_index("Concept ID")["Concept Name"]
            .to_dict()
        )
        self.concepts_id_to_name = dict(
            [(str(concept), name) for (concept, name) in concepts_id_to_name.items()]
        )
