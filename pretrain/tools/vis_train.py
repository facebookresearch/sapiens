...
def main():
    ...
    # Set the train batch size to be read by shutterstock dataset class
    ...
    dataset_iter = iter(runner.train_dataloader.dataset)

    for i in range(num_samples):
        data = next(dataset_iter)
        print(f'Processing {i}/{num_samples}')

        image = data['inputs'] ## [3, H, W]
        image = image.permute(1, 2, 0).cpu().detach().numpy()
        save_path = os.path.join(cfg.work_dir, f'{i:06d}.jpg')
