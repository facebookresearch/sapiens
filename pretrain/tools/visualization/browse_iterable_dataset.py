...
def test_airstore():
    ...
    scratch_dir = '/home/rawalk/drive/mmpretrain/Outputs/train'
    os.makedirs(scratch_dir, exist_ok=True)

    from mmpretrain.datasets import Shutterstock
    ...
    # let's build a dataloader!
    # NOTE: we are handling data on the other side?
    loader = DataLoader(
        dataset,
        batch_size=4,
        shuffle=False,
    ...
    for b, batch in enumerate(loader):
        ...
        B = batch["img"].shape[0]
        image = batch["img"]
        print(f"{batch['img'].shape} {batch['text']} {image.max()} {image.min()}")
        for i in range(B):
            np_image = (
                batch["img"][i].permute(1, 2, 0).cpu().numpy().astype(np.uint8).copy()
            )
            text = batch["text"][i]
            cv2.putText(
                np_image,
            ...
            cv2.imwrite(f"{scratch_dir}/{b}_{i}.png", np_image[..., ::-1].copy())
