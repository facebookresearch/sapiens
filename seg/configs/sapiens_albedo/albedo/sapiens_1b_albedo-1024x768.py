...
model = dict(
    ...
    backbone=dict(
        ...
        out_type='featmap',
        patch_cfg=dict(padding=2), ## probably not needed
        init_cfg=dict(
            type='Pretrained',
...
default_hooks = dict(
    ...
    visualization=dict(type='AlbedoVisualizationHook', interval=vis_every_iters, max_samples=4, vis_image_width=384, vis_image_height=512),
    )
...
train_pipeline = [
    ...
    dict(
        ...
        ratio_range=(0.2, 2.0),
        keep_ratio=True),
    dict(type='RandomAlbedoResizeCompensate'), ## compensate for the random resize augmentation
    dict(type='RandomAlbedoCrop', crop_size=(1024, 768)), ## height, width. works for normal as well
    dict(type='AlbedoResize', scale=(768, 1024)), ## in case if image was too small and random crop returned the original image
    dict(type='AlbedoRandomFlip', prob=0.5,),
    dict(type='AlbedoRandomRotate', prob=0.5, degree=60, albedo_pad_val=-1000),
    dict(type='GenerateAlbedoTarget', background_val=-1000), ## this should be less than used in the loss functions
    dict(type='RandomBrightness'),
    dict(type='PackAlbedoInputs')
]
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        metainfo=dict(from_file='configs/_base_/datasets/albedo.py'),
