...
pretrained_checkpoint = (
    "../pretrain/checkpoints/sapiens_1b/sapiens_1b_epoch_173_clean.pth"
)
...
model = dict(
    ...
    decode_head=dict(
        ...
        deconv_out_channels=(
            ...
            768,
            768,
        ),  ## this will 2x at each step. so total is 8x
...
param_scheduler = [
    dict(
        type="LinearLR", start_factor=1e-3, begin=0, end=400, by_epoch=False
    ),  # warm-up
    dict(
        type="PolyLR",
        eta_min=0.0,
...
train_cfg = dict(
    ...
    val_interval=evaluate_every_n_epochs,
)
...
default_hooks = dict(
    ...
    visualization=dict(
        type="GeneralSegVisualizationHook",
...
train_pipeline = [
    ...
    dict(
        type="RandomCrop", crop_size=(1024, 768), cat_max_ratio=0.75
    ),  ## height, width
    dict(
        type="Resize", scale=(768, 1024), keep_ratio=False
    ),  ## in case if image was too small and random crop returned the original image
    dict(
        type="RandomRotate", prob=0.5, degree=45, seg_pad_val=0
    ),  ## the black pixels are set as background
    dict(
        type="RandomFlip", prob=0.5, swap_seg_labels=[(3, 4), (5, 6), (7, 8)]
    ),  ## for the new 28 classes,
...
test_pipeline = [
    ...
    dict(
        type="Resize", scale=(768, 1024), keep_ratio=False
    ),  ## this is width x height, 768 x 1024
