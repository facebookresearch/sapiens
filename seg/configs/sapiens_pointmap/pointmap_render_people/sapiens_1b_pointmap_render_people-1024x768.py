...
model = dict(
    ...
    decode_head=dict(
        type='VitPointmapHead',
        in_channels=embed_dim,
        # channels=512,
        # deconv_out_channels=None,
        # deconv_kernel_sizes=None,
        # upsample_factor=16, ## F.interpolate by 16x
...
default_hooks = dict(
    ...
    visualization=dict(type='PointmapVisualizationHook', interval=vis_every_iters, max_samples=4, vis_image_width=384, vis_image_height=512),
    )
...
train_pipeline = [
    ...
    dict(type='PackPointmapInputs', meta_keys=('img_path', 'ori_shape',
...
test_pipeline = [
    dict(type='LoadImage'),
    dict(type='Resize', scale=(768, 1024), keep_ratio=False), ## this is width x height,
    # dict(type='Resize', scale=(768, 1024), keep_ratio=True), ## this is width x height,
    # dict(type='PadPointmap', size=(768, 1024)), ## width x height, 768 x 1024
