...
class HiCaWrinkleTexDataset(BaseSegDataset):
    def __init__(self,
                 **kwargs) -> None:
    ...
    def load_data_list(self) -> List[dict]:
        ...
        print('\033[92mDone! HiCaWrinkleTex. Loaded total samples: {}\033[0m'.format(len(data_list))) ## 98424 images train
    ...
    def get_data_info(self, idx):
        ...
        if not os.path.exists(wrinkle_tex_path) or not os.path.exists(mask_path):
            return None

        wrinkle_tex = cv2.imread(wrinkle_tex_path)
        wrinkle_tex = cv2.cvtColor(wrinkle_tex, cv2.COLOR_BGR2RGB) ## rgb. we learn rgb -> rgb transform. 

        # Normalize mean_tex to the range 0-1
        wrinkle_tex = wrinkle_tex.astype(float) / 255.0
