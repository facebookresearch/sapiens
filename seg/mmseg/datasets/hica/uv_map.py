...
class HiCaUVMapDataset(BaseSegDataset):
    def __init__(self,
                 **kwargs) -> None:
    ...
    def load_data_list(self) -> List[dict]:
        ...
        print('\033[92mDone! HiCaUVMap. Loaded total samples: {}\033[0m'.format(len(data_list))) ## 98424 images train

        return data_list
    
    def load_uv(self, uv_img_path):
        uv = typed.load(uv_img_path) ## 960 x 720 x 4
    ...
    def get_data_info(self, idx):
        ...
        if not os.path.exists(uv_map_path) or not os.path.exists(mask_path):
            return None

        uv_map = self.load_uv(uv_map_path) # (h, w, 2) & float32
        ...
        data_info = {
            ...
            'img_id': '',
            'img_path': '',
            'gt_val': uv_map, ## 2 channels, float32
            'mask': mask,
            'id': idx,
            'bbox': bbox,
