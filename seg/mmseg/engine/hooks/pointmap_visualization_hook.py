...
class PointmapVisualizationHook(Hook):
    def __init__(self,
                 draw: bool = False,
                 interval: int = 50,
                 show: bool = False,
    ...
    def after_train_iter(self,
        ...
        for i, (input, data_sample_with_pred, seg_logit) in enumerate(zip(inputs, data_samples_with_pred, seg_logits)):
            ...
            K = data_sample_with_pred.K ## intrinsics 3 x 3

            gt_pointmap = data_sample_with_pred.gt_depth_map.data.numpy() ## 3 x H x W
            gt_pointmap = gt_pointmap.transpose((1, 2, 0)) ## H x W x 3

            mask = gt_pointmap[:, :, 0] > -100  # Assuming the first channel indicates valid points
            vis_gt_pointmap = self.vis_point_map(gt_pointmap, mask)

            # Resize prediction to the size of the image
            pred_pointmap = cv2.resize(seg_logit, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_LINEAR)
            vis_pred_pointmap = self.vis_point_map(pred_pointmap, mask)  # Visualizing the first channel as an example

            vis_image = np.concatenate([image, vis_gt_pointmap, vis_pred_pointmap], axis=1)
