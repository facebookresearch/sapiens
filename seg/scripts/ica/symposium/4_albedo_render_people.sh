...
if [ $# -eq 3 ]; then
    ICA_DIR="$1"
    JOBS_PER_GPU="$2"
    IFS=' ' read -r -a VALID_GPU_IDS <<< "$3"  # This reads the third argument as an array
fi
...
#--------------------------MODEL CARD---------------
MODEL_NAME='sapiens_1b'; CHECKPOINT='/uca/rawalk/sapiens_host/albedo/checkpoints/sapiens_1b/sapiens_1b_albedo_render_people_epoch_42.pth'
