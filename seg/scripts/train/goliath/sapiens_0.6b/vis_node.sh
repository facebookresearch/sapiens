...
###--------------------------------------------------------------
CONFIG_FILE=configs/vis_sapiens_seg/${DATASET}/${MODEL}.py
...
if [ "$mode" = "debug" ]; then
    ...
    CUDA_VISIBLE_DEVICES=${DEVICES} python tools/vis_train.py ${CONFIG_FILE} --work-dir ${OUTPUT_DIR} --cfg-options ${OPTIONS}
