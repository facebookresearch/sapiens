...
def main():
    ...
    for i, image_name in tqdm(enumerate(image_names), total=len(image_names)):
        ...
        wrinkle_tex = result.transpose(1, 2, 0) ### (H, W, C)

        ## clip in the pred to [0, 1]
        wrinkle_tex = np.clip(wrinkle_tex, 0, 1)

        ##----------------------------------------
        wrinkle_tex = (wrinkle_tex * 255).astype(np.uint8)
