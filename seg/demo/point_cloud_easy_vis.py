...
def main():
    ...
    for i, image_name in tqdm(enumerate(image_names), total=len(image_names)):
        ...
        ## normalize the foreground depth to 0 to 1. 0 is close to camera, 1 is far from camera
        depth_foreground = (depth_foreground - min_val) / (max_val - min_val)

        ## in metres. 50 mm lens is common ## suits most images
        ## figure out the rough distance of the human from camera. width of the human roughly
        # focal_length = 85 * 1e-3
        focal_length = 1000

        shift = 1.5 ## distance to near point from camera
        scale = 3 ## tip of face to back shoulder

        depth_foreground = depth_foreground * scale + shift
        depth_normalized[mask > 0] = depth_foreground ## 0 is near camera, 1 is far from camera

        ##----------------------------------------------------
        # Create a point cloud from the depth map for the foreground and color it using the pixel colors
        rows, cols = depth_normalized.shape
        ...
        foreground_mask = (mask > 0) & (depth_normalized != np.inf)
        ...
        depth_normalized = depth_normalized[row_indices, :][:, col_indices]
        image = image[row_indices, :][:, col_indices, :]

        # Convert depth to 3D points
        z = depth_normalized[foreground_mask]
